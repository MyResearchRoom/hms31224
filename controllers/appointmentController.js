...
const patientController = {
  ...
  async submitAppointment(req, res) {
    ...
    const { followUp, note, fees, investigation, chiefComplaints, diagnosis, prescription } = req.body;
    ...
    try {
      const appointment = await Appointment.findByPk(appointmentId, {
        attributes: ["id", "followUp", "note", "fees", "investigation", "chiefComplaints", "diagnosis"],
      });
      if (!appointment) {
        return res.status(404).json({ error: "Appointment not found" });
      }
      ...
      appointment.prescription = Array.isArray(prescription) ? prescription : null;
