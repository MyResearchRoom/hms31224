...
const receptionistController = {
    ...
    try {
      ...
      const data = await Notification.create({
        ...
        // } checked in at ${new Date().toLocaleString()}.`,
          } checked in at ${momentTimezone().tz("Asia/Kolkata").format("hh:mm A, DD MMM YYYY")}.`,
    ...
    try {
      ...
      const data = await Notification.create({
        ...
        // } checked out at ${new Date().toLocaleString()}.`,
          } checked out at ${momentTimezone().tz("Asia/Kolkata").format("hh:mm A, DD MMM YYYY")}.`,
  ...
  async getReceptionistAttendanceStats(req, res) {
    ...
    try {
      ...
      let startOfMonth = moment(`${currentYear}-${currentMonth}-01`)
        .startOf("month")
        .toDate();
      ...
      const avgCheckInTime = totalAttendance > 0 ? new Date(totalCheckInTime / totalAttendance)
        .toLocaleTimeString("en-US", { timeZone: "Asia/Kolkata", hour12: false })
        : null; // HH:MM:SS-
