...
const doctorController = {
  ...
  async verifyEmail(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: "Verify email",
          details: `Email varification for User Id: ${user.id}, email: ${user.email}`,
          hospitalId: user.id,
          receptionistId: null,
          doctorId: user.id,
  ...
  async removeEmail(req, res) {
    ...
    try {
      ...
      //   {
      //     action: "Remove email",
      //     details: `Email varification for User ${user.id}`,
      //     hospitalId: user.id,
      //     receptionistId: null,
      //     doctorId: user.id,
      //     ipAddress: req.clientIp,
  ...
  async login(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: "Login",
          details: `User (${userType}) trying login with otp, ID: ${user.id}, email: ${user.email}`,
          hospitalId,
  ...
  async acceptTermsAndConditions(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: "Accept terms & conditions",
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) accepted terms & conditions.`,
          hospitalId: doctor.id,
          receptionistId: null,
          doctorId: doctor.id,
          ipAddress: req.clientIp,
      ...
      await AuditLog.create(
        {
          ...
          hospitalId: userType === "doctor" ? user.id : user.doctorId,
          receptionistId: userType === "receptionist" ? user.id : null,
          doctorId: userType === "doctor" ? user.id : null,
          ipAddress: req.clientIp,
          userAgent: req.headers["user-agent"],
      ...
      await AuditLog.create(
        {
          ...
          hospitalId: decoded.role === "doctor" ? user.id : user.doctorId,
  ...
  async setFees(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: `${existingFee ? "Update" : "Add"} fee`,
  ...
  async deleteFees(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: "Delete fee",
          details: `User (ID: ${doctor.id}, email: ${doctor.email}), deleted the fee (${fee.feesFor})`,
          hospitalId: doctor.id,
          receptionistId: null,
          doctorId: doctor.id,
          ipAddress: req.clientIp,
  ...
  async paymentScanner(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: `${updated ? "Update" : "Add"} payment QR`,
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) ${
  ...
  async addSignature(req, res) {
    ...
    try {
      const doctor = await Doctor.findByPk(req.user.id, {
        attributes: ["id", "signature", "signatureContentType", "email"],
        transaction,
      });
      ...
      await AuditLog.create(
        {
          action: `${updated ? "Update" : "Add"} signature`,
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) ${
            updated ? "updated" : "added"
          } signature.`,
          hospitalId: req.user.hospitalId,
  ...
  async addLogo(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: `${updated ? "Update" : "Add"} logo`,
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) ${
            updated ? "updated" : "added"
          } logo.`,
          hospitalId: req.user.hospitalId,
  ...
  async setCheckInOutTime(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: `${updated ? "Update" : "Add"} check in/out time`,
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) requested a password reset.`,
          hospitalId: req.user.hospitalId,
          receptionistId: null,
          doctorId: doctor.id,
          ipAddress: req.clientIp,
      ...
      await AuditLog.create(
        {
          ...
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) updated the information.`,
          hospitalId: req.user.hospitalId,
          receptionistId: null,
          doctorId: doctor.id,
          ipAddress: req.clientIp,
  ...
  async setClinicTime(req, res) {
    ...
    try {
      ...
      await AuditLog.create(
        {
          action: `${updated ? "Update" : "Add"} clinic start/end time`,
          details: `User (ID: ${doctor.id}, Email: ${doctor.email}) ${
            updated ? "updated" : "added"
